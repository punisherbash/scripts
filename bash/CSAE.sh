...
"7")
	# shodan
	echo "scan $ip with Shodan...."
	echo "scan $ip with Shodan...." > report_$ip.txt
	#shodan domain $ip >> report_$ip.txt
	shodan host $ip --history >> report_$ip.txt
	echo "Finishing the scan Shodan" >> report_$ip.txt
	echo "Finished Shodan"
;;
"8")
	# AbuseIP-DB
